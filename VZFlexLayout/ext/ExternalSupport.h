...
#define MAIN_CALL ExMainCall_O2OReact // MAIN_CALL(tag, delayTime, priority, block)
//#error ThisModuleName需要替换为自己模块名（区分大小写），替换后注释该错误提示。------ （第3个替换）------
#define MAINA_CALL ExMainACall_O2OReact  // MAINA_CALL(tag, delayTime, priority, block)
